...
## 2026-06-14 - Unihan stroke table packed to a gzipped binary

Repacked the bundled Unicode Unihan `kTotalStrokes` table (the CJK total-stroke lookup behind `_name_values/cjk_unihan_strokes`) from a 98,060-line text file into a packed, gzipped binary, cutting it from 837,758 to 21,135 bytes — 39.6x on disk, and inside the wheel from ~233 KB (deflated text) to ~21 KB. The encoding was chosen empirically, not by guess: the sorted code points are delta-varint (ULEB128) encoded — because they are sorted and largely contiguous, most deltas are 1 — each followed by one stroke byte (max stroke is 84, fits a byte), gzip-compressed with `mtime=0` so the output is reproducible; the header is a `UTS1` magic plus a uint32 entry count. Four candidates were measured before choosing: text->gz 238 KB, fixed 4-byte/entry->gz 172 KB, columnar u32+u8->gz 156 KB, and delta-varint+u8->gz 21 KB; the last is ~11x smaller than gzipped text purely from the delta structure. `encode_table`/`decode_table` live in the loader module and are shared with `tools/unihan/generate_total_strokes.py` (now writing and `--check`-ing bytes); the loader decodes lazily and caches for the process lifetime. Added `data/UNIHAN-NOTICE.txt` to carry the Unicode source/version/license attribution the binary can no longer hold inline, and removed the `.txt`.

Verification: exact round-trip (decoded dict equals the original text table), deterministic re-encode (so `--check` stays byte-stable), fresh-process load through `importlib.resources`, and — since shipping the data is the whole point — a real hatchling wheel build confirmed to contain `unihan_total_strokes.bin.gz` (21,135 B) plus the notice and not the old text. Full suite 372 passed, ruff + mypy clean.

The whole CJK-Unihan provider (`cjk_unihan_strokes.py`, `data/`, `cjk_name_strokes/parsers.py`, `tools/unihan/`, and the cjk tests) had been sitting as **untracked working-tree WIP** — never committed in any PR — and was also **half-wired** (the package did not re-export the parsers, and the tests assumed a stroke source the engine lacked). Rather than patch it, the stroke layer was reworked into a **provider registry**, which is what the design note's `StrokeCountProvider` always intended. A `StrokeCountProvider` (a `Protocol`: `id`, `version`, `stroke_count(char) -> int | None`) is looked up by name in a `StrokeProviderRegistry`; the bundled Unihan table is registered as `"unihan"` and is the default, so a reading needs only `surname`/`given_name`. The stringly-typed `strokes` option, `stroke_source` enum, and `to_strokes_option` were **removed**; `stroke_source` now just names the provider. Callers wrap a parsed third-party dataset in a `MappingStrokeProvider` and `register_provider(...)` it on the process-wide default registry, or pass a private registry via `build_engine(registry=...)`; the provider's id/version are stamped as the reading's `value_system`, and the resolved per-character counts are recorded in the `values` modifier, so a reading is reproducible and provenance-traceable. Full suite 378 passed; ruff + mypy + `mkdocs --strict` clean. Committed on `cjk-unihan-stroke-data` (PR #10).

Findings: (1) The generator's `--check` needs the Unihan archive, which is not cached locally (`tools/unihan/sources/` holds only a manifest + README); deterministic encoding makes a later `--check --download-missing` safe. (2) Python wheels are zip/DEFLATE, so a pre-gzipped binary is what actually shrinks the distributed artifact and keeps the installed footprint small — storing raw delta-varint bytes would let zip recompress to ~21 KB in the wheel but leave the on-disk installed file at ~196 KB. (3) No third-party stroke data is bundled (only the Unicode-licensed Unihan table); KANJIDIC2/KanjiVG are share-alike (CC BY-SA 4.0 / 3.0), whose ShareAlike terms preclude bundling here, so the parsers convert caller-obtained data instead.
...
## 2026-06-14 - cjk_name_strokes: bundled Unihan is now the default stroke source

Per maintainer direction ("upstream should no longer need explicit stroke counting as a parameter"), `cjk_name_strokes` no longer requires a `strokes` option. Added `StrokeSource.UNIHAN`, which resolves counts from the already-bundled packed Unihan `kTotalStrokes` table (`_name_values/cjk_unihan_strokes`). When no `strokes` is supplied the engine defaults to UNIHAN (a per-character `strokes` override still wins); the explicit `request` and third-party `provider` paths are unchanged. The reading records this distinctly (`stroke_source=unihan`, `value_system=cjk_unihan_strokes.v1`), so a Unihan-sourced reading is never mistaken for a Kangxi-based seimei-handan school count.

This relaxes the decision above ("Unicode data ... should still be an explicit generic source, not the default school source"). The tension is resolved by provenance, not input: Unihan is the default only when the caller supplies no counts, and the `stroke_source`/`value_system` make explicit that it is the generic Unicode source rather than a school's counts. If strict school fidelity by default is preferred, revert the default to REQUEST and require an explicit `stroke_source=unihan`. The existing cjk engine tests all pass `strokes` explicitly, so they keep `stroke_source=request`; full gate green (ruff + mypy, 377 tests).
